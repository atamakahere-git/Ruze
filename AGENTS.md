...
A lightweight, high-performance Discord–Minecraft chat bridge bot, themed after Reze from *Chainsaw Man*. Built in Rust with `poise`/`serenity` for Discord, tailing the Minecraft server's `latest.log` directly — no mods or plugins required.
...
- [Module Reference](#module-reference)
  - [`src/main.rs`](#srcmainrs) — Entry point & orchestration
  - [`src/consts.rs`](#srcconstsrs) — Configuration loading
  - [`src/log_parser.rs`](#srclog_parsersrs) — Log parsing
  - [`src/rcon.rs`](#srcrconrs) — RCON client
  - [`src/stats.rs`](#srcstatsrs) — Player stats tracking
  - [`src/storage.rs`](#srcstoragers) — Persistence layer
  - [`src/bot/mod.rs`](#srcbotmodrs) — Bot root & error types
  - [`src/bot/types.rs`](#srcbottypesrs) — Data types & event formatting
  - [`src/bot/handler.rs`](#srcbothandlerrs) — Event handler & forwarding
  - [`src/bot/commands.rs`](#srcbotcommandsrs) — Slash/prefix commands
- [Data Flow](#data-flow)
...
6. **Storage** — Open the `redb` database at `$XDG_STATE_HOME/ruze/ruze.redb`.
...
|---|---|---|
| 1 (lowest) | System-wide | `/etc/ruze.toml` |
| 2 | Per-user XDG | `$XDG_CONFIG_HOME/ruze.toml` (→ `~/.config/ruze.toml`) |
| 3 | Per-user home | `$HOME/.ruze.toml` |
| 4 (highest) | Environment | `RUZE_*` variables (with deprecated old-name fallback) |
...
- `Config::overlay_env()` — Reads `RUZE_*` env vars (and deprecated fallbacks).
...
- `default_db_path()` — Returns `$XDG_STATE_HOME/ruze/ruze.redb`.
...
- `event_handler(ctx, event, data)` — Serenity event handler dispatched by the framework. Handles:
  - `Message` — Processes incoming Discord messages: checks they're in the bridged channel, processes mentions, checks silent prefixes, forwards to `dc_event_tx`.
...
|---|---|---|
| `~ping` | Everyone | "UwU Helloo!" — liveness check |
...
Minecraft chat "@s CONFIRM-<code>"
  ...
  → handler::forward_mc_events() detects CONFIRM- in content
    ...
    → storage::connect_accounts(discord_id, mc_username)
...
[storage]
# database_path = "/custom/path/ruze.redb"   # optional override
...
|---|---|---|
| `RUZE_DISCORD_TOKEN` | Yes | — |
| `RUZE_LOG_PATH` | Yes | — |
| `RUZE_RCON_PASSWORD` | Yes | — |
| `RUZE_RCON_ADDRESS` | No | `localhost:25575` |
| `RUZE_MC_SERVER_ADDRESS` | No | `localhost:25565` |
| `RUZE_OWNER_ID` | Yes | — |
| `RUZE_GUILD_ID` | No | — |
| `RUZE_DATABASE_PATH` | No | `$XDG_STATE_HOME/ruze/ruze.redb` |
...
- `storage.rs` — Integration tests using temporary redb files for bridge CRUD, account linking, and parent directory creation.
...
```ini
[Unit]
Description=RUZE Discord-Minecraft Bridge
After=network.target minecraft.service

[Service]
Type=simple
User=minecraft
Environment=RUZE_DISCORD_TOKEN=your_token
Environment=RUZE_LOG_PATH=/var/minecraft/logs/latest.log
Environment=RUZE_RCON_PASSWORD=your_password
Environment=RUZE_OWNER_ID=123456789012345678
ExecStart=/usr/local/bin/ruze
Restart=on-failure
RestartSec=10
...
FROM alpine:latest
COPY --from=builder /app/target/release/ruze /usr/local/bin/ruze
CMD ["ruze"]
```
...
| Path | Purpose |
|---|---|
| `$XDG_CONFIG_HOME/ruze/ruze.toml` | User configuration |
| `$XDG_STATE_HOME/ruze/ruze.redb` | redb database (bridge state, accounts, stats) |
| `$XDG_DATA_HOME/ruze/bridge_state.toml` | (Legacy) Deprecated bridge state file |
...
- **Atomic file writes** — Legacy bridge state writes use write-to-tmp-then-rename (deprecated in favor of redb).
...
|---|---|---|
| Bot doesn't start | Missing config field | Check `RUZE_*` env vars or config file |
...
| `rust-mc-status` | 2.0 | Server List Ping (MOTD, latency, icon) |
...
| `rand` | 0.9 | Verification code generation |